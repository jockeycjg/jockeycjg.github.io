...
本章节展示了不同阶段的测试边界与协同关系。

### 2.1 核心架构对比

| 维度             | Unit/L1 集成阶段                       | Pre-L3 集成阶段                               |
| :--------------- | :------------------------------------- | :-------------------------------------------- |
| **核心目标**     | 端到端特性集成验证与无线电健壮性评估   | 评估底层就绪度 (面向 RAN/LTE/5G/Factory 发布) |
...
Unit/L1 阶段作为第一道防线，重点关注基本功能可用性。Pre-L3 阶段承接 Unit/L1 的输出，在真实无线电环境下进行高级别验收。
...
| 核心功能就绪                                                 | 性能与校准                                                   |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| • 小区射频发射开启 (Cell On Air)<br>• L3 控制面呼叫建立<br>• 分离模式呼叫验证 | • 用户面数据吞吐量 (256QAM)<br>• 天线波束赋形校准<br>• 测试模型验证 |

| 控制与健壮性                                                | 生命周期维护                                                 |
| :---------------------------------------------------------- | :----------------------------------------------------------- |
| • 模块及站点复位恢复<br>• 载波锁定/解锁管理<br>• 冷启动恢复 | • 最大4个版本的兼容性升级<br>• RF SW 升级与回滚安全机制<br>• 版本回退保护 |
...
真实无线电设备在传导模式下与自动化环境无缝对接。采用商用终端模拟器与 MIMO 信道，结合高精度共享硬件，确保射频输出符合 3GPP 规范。
